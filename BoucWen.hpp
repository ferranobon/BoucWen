...
namespace BoucWen {
     namespace NewtonRhapson {
	  ...
	  public:	       
	       BoucWen();
	       BoucWen(precision_t *const parameters);
	       ~BoucWen();
	       void List() const;
	       precision_t Solver(const precision_t Value);
	       precision_t signum(const precision_t Value);
	  };
	  
	  template <typename precision_t>
	  BoucWen<precision_t>::BoucWen(precision_t *const parameters)
	  {
	       alpha = parameters[0];
	       Fy = parameters[1];
	       ko = parameters[2];
	       ...
	       maxIter = 100; tolerance = (precision_t) 1E-12; startPoint = (precision_t) 0.01;     
	       DispT = (precision_t) 0.0; e_old = (precision_t) 0.0; z_old = (precision_t) 0.0;
	  }
	  
	  template <typename precision_t>
	  BoucWen<precision_t>::~BoucWen()
	  {
	       alpha = (precision_t) 0.0;
	       Fy = (precision_t) 0.0;
	       ko = (precision_t) 0.0;
	       beta = (precision_t) 0.0;
	       gamma = (precision_t) 0.0;
	       n = (precision_t) 0.0;
	  }
	  ...
	  {
	       if (Value > (precision_t) 0.0 ){
		    return (precision_t) 1.0;
	       } else if (Value < (precision_t) 0.0){
		    return (precision_t) -1.0;
	       } else return (precision_t) 0.0;
	  }
	  ...
	  precision_t BoucWen<precision_t>::Solver(const precision_t DispTdT)
	  {
	       ...
	       count = 0;
	       z_new = (precision_t) 1.0;
	       z_new_p = startPoint; z_eval = startPoint;
	       
	       while (std::fabs(z_new_p - z_new) > tolerance && count < maxIter ){
		    e_new = e_old + ((precision_t) 1.0 - alpha)*deltaDisp*ko/Fy*z_eval;

		    sign = signum(deltaDisp*z_eval);
		    Psi = gamma + beta*sign;
		    Phi = (precision_t) 1.0 - std::pow(std::fabs(z_eval),n)*Psi;
		    ...
		    e_new_ = ((precision_t) 1.0 - alpha)*ko/Fy*deltaDisp;
		    sign = signum(z_eval);
		    Phi_ = -n*std::pow(std::fabs(z_eval), n - (precision_t) 1.0)*sign*Psi;
		    fz_new_ = (precision_t) 1.0 - Phi_*deltaDisp*ko/Fy;
		    ...
		    force = alpha*ko*DispTdT + ((precision_t) 1.0 - alpha)*Fy*z_eval;
		    
		    // Compute material degradation parameters.
		    e_new = e_old + ((precision_t) 1.0 - alpha)*ko/Fy*deltaDisp*z_eval;
	       }
	  ...
	  precision_t BoucWen_Deg<precision_t>::Solver(const precision_t DispTdT)
	  {
	       ...
	       while ( fabs(z_new_p - z_new) > tolerance && count < maxIter ){
		    e_new = e_old + ((precision_t) 1.0 - this->alpha)*deltaDisp*this->ko/this->Fy*z_eval;
		    ...
		    /* Evaluate function derivatives with respect to z_eval for the Newton-Rhapson scheme */
		    e_new_ = ((precision_t) 1.0 - this->alpha)*this->ko/this->Fy*deltaDisp;
		    A_new_ = -deltaA*e_new_;
		    nu_new_ = deltaNu*e_new_;
		    eta_new_ = deltaEta*e_new_;
		    sign = this->signum(z_eval);
		    Phi_ = A_new_ - this->n*pow(fabs(z_eval), this->n - (precision_t) 1.0)*sign*Psi*nu_new - pow(fabs(z_eval), this->n)*Psi*nu_new_;
		    fz_new_ = (precision_t) 1.0 - (Phi_*eta_new - Phi*eta_new_)/pow(eta_new, (precision_t) 2.0)*deltaDisp*this->ko/this->Fy;
		    ...
		    // Compute restoring force.
		    force = this->alpha*this->ko*DispTdT + ((precision_t) 1.0 - this->alpha)*this->Fy*z_eval;

		    // Compute material degradation parameters.
		    e_new = e_old + ((precision_t) 1.0 - this->alpha)*this->ko/this->Fy*deltaDisp*z_eval;
		    A_new = A0 - deltaA*e_new;
		    nu_new = nu0 + deltaNu*e_new;
		    eta_new = eta0 + deltaEta*e_new;
	       }
